...
if docs != [] and reporte == "Diario - Pedidos":
  ...
  for date in unique_dates:
      ...
      # Format the 'Venta Producto ($)' column, only for the TOTAL row
      daily_summary.loc[daily_summary['Item Description'] == 'TOTAL', 'Venta Producto ($)'] = daily_summary['Venta Producto ($)'].apply(lambda x: f"$ {x:,.2f}")
